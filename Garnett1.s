...
@ Class: CS 413-01
@ Term: Spring 2021
@ Objective: The purpoes of this lab is to provide students with experience with the ARM Thumb mode 
@ with assembly programming
@ History:
@	Created 02/06, adding comments when necessary
@	Copied on 3/06, editing to comly with the requirements of Lab3
@
@ Use these commands to assemble, link, run and debug this program:
@    as -o Garnett1.o Garnett1.s
@    gcc -o Garnett1 Garnett1.o
@    ./Garnett1 ;echo $?
@    gdb --args ./Garnett1
...
main:
@ This first portion just tells the processor to enter Thumb mode
   ldr r0, =startthumb + 1
   bx r0
...
@ case will be intInput. 
   ...
   str r1, [r5]			@ Store the input into array 
   add r5, r5, #4		@ Move to next value in the array
   sub r4, r4, #1		@ Subtract 1 from r4, to note that one input as been read in.
...
@*******************
   ...
   ldr r4, [r1]			@ Load value of index i of arrOne into r4
   add r1, r1, #4		@ Increment to element i+1 of arrOne
   ldr r5, [r2]			@ Load value of index i of arrTwo into r5
   add r2, r2, #4		@ Increment to element i+1 of arrTwo
   add r6,r4,r5			@ r6 <- arrOne[i] + arrTwo[i]
   str r6,[r3]			@ arrThree[i] <- r6
   add r3, r3, #4		@ Increment to next element in arrThree
   sub r0, r0, #1		@ Subtract 1 from r0 to note one element of r3 has been set
...
loop:
   ldr r1,[r4]			@ Load the value pointed at by r4 into r1
   add r4, r4, #4		@ Increment r4 to point to the next value in the array
   ldr r0,=element		@ Load r0 with the format for printf
   bl printf			@ Print
   sub r5,r5,#1		
   bne loop 
   pop {pc}
